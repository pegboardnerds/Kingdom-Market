...
function getData(incrementing, bias) {
  //_localdata = [];
  $.ajax({
    url: 'database.json',
    type: 'POST',
    ...
    success: function (data) {
      ...
      lofx = x;
      if (bias) { if (x == databaselastamount) { return true; } }
      if (incrementing) { x -= databaseshown; }else{Reset();}
      let until = databaseshown + 5;
      lastamount = x;
      for (listing in data) {
        if (databaseshown == until || x < 0) { return; }
        displayAllPost(data[x]["name"], data[x]["details"],
          data[x]["image"], data[x]["cost"],
          data[x]["datecreated"], data[x]["sellerid"], data[x]["reference"],
          data[x]["sellerusername"]); _awaitpost = 0;
        databaseshown++;
        x--;
      }
    },
