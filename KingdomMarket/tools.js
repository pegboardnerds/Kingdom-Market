let background = null;
function getel (text){ return document.getElementById(text); }
function HasLetters(text) {
    const abc = "abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ";
    for (x = 0; x < text.length; x++) {
        if (abc.includes(text[x])) { return true; }
    }
    return false;
}
...
function darkenBackground(elem){
    if(background==null){
...
function smallWindow(){if(window.innerWidth<600){return true}else{return false;}}
function isEmpty(value){if(HasLetters(value)||HasNumbers(value)){return false;}return true;}
